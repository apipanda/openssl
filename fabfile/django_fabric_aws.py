...
def update_packages():
    ...
    requirements/prod.txt
    """
    _run_task(tasks.update_packages, "Updating server packages with pip...",
              "Finished updating python packages")
...
def reload_nginx():
    ...
    """
    _run_task(tasks.reload_nginx, "Reloading the nginx config files...",
              "Finished reloading nginx")
...
def _virtualenv_command(command):
    ...
    """
    with cd(fabconf['APPS_DIR']):
        sudo(fabconf['ACTIVATE'] + ' && ' + command,
             user=fabconf['SERVER_USERNAME'])
