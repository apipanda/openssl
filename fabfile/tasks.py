...
configure_instance = [
    ...
    {"action": "sudo",
        "params": "mv /home/%(SERVER_USERNAME)s/boto.cfg /etc/boto.cfg"},

    # virtualenvwrapper
    {"action": "sudo",
        "params": "mkdir %(VIRTUALENV_DIR)s", "message": "Configuring virtualenvwrapper"},
    {"action": "sudo",
    ...
    # webapps dir
    {"action": "sudo",
        "params": "mkdir %(APPS_DIR)s", "message": "Creating webapps directory"},
    ...
    {"action": "run",
        "params": "chmod 600 /home/%(SERVER_USERNAME)s/.ssh/%(BITBUCKET_DEPLOY_KEY_NAME)s"},
    {"action": "run",
    ...
    # Clone the git repo
    {"action": "run",
        "params": "git clone %(BITBUCKET_REPO)s %(PROJECT_PATH)s"},
    ...
    {"action": "virtualenv",
        "params": "pip install -r %(PROJECT_PATH)s/requirements/common.txt --upgrade"},
    {"action": "virtualenv",
        "params": "pip install -r %(PROJECT_PATH)s/requirements/prod.txt --upgrade"},
